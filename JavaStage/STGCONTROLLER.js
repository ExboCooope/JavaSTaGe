...
function snapshotInput(){
    ...
    for(var i=0;i<16;i++){
        _key_snapshot[i]=0;
        if(_key_map[i])
        for(var j=0;j<_key_map[i].length;j++){
            if(_key_pool[_key_map[i][j]])_key_snapshot[i]=1;
        }
    }
}
...
function  _stgMainLoop_SendInput(){
    snapshotInput();
   // if(!stg_in_replay){
        var t=_packInput(_key_snapshot);
        if(t){
            t=t;
        }
        var th=_key_sync_pool[_key_sync_id_head];
        _sendInput(_key_sync_id_head,t);

        _receiveInput(stg_local_player_pos,th,t);
...
function _sendInput(iSync,iInput){
    if(stg_mp_status!=2)return;
    var th=_key_sync_pool[iSync];
   // console.log("Sending input sync ="+th);
    mpSendData({ipt:1,code:iInput,slt:stg_local_player_pos,sync:th});
}

function _receiveInput(iPlayerSlot,iSync,iInput){
    var i;
    var j;
    i=_key_bottom;
    j=0;
